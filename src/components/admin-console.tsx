...
export function AdminAuthRequired() {
  return (
    <ConsoleLayout title="관리자" subtitle="플랫폼 운영 콘솔" links={adminLinks}>
      <ConsoleSection>
        <h2 className="text-2xl font-bold">어드민 권한이 필요합니다</h2>
        <p className="mt-2 text-sm text-content-secondary">어드민 계정으로 로그인한 사용자만 플랫폼 운영 콘솔에 접근할 수 있습니다.</p>
      </ConsoleSection>
    </ConsoleLayout>
  );
}
        ...
        <ConsoleSection title="운영 알림" description="정산, 배송, 마켓 상태와 관련된 고위험 이슈입니다.">
          <div className="grid gap-3">
            {dashboard?.alerts.map((alert) => (
              <div key={alert.id} className="rounded-md bg-surface-subtle p-3">
                <div className="flex justify-between gap-4">
                  <p className="font-bold">{alert.title}</p>
                  <StatusBadge value={alert.severity} />
                </div>
                <p className="mt-1 text-sm text-content-secondary">{alert.description}</p>
              </div>
            ))}
          </div>
        </ConsoleSection>
  ...
  return (
    <ConsoleLayout title="관리자" subtitle="플랫폼 운영 콘솔" links={adminLinks}>
      ...
      <ConsoleSection className="mt-5" title="쿠폰 발급 대상" description="발급 버튼을 누르면 선택된 회원에게 발급하는 흐름으로 처리됩니다.">
        <Input
          className="h-11 w-full rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold md:w-96"
          type="number"
          min={1}
        ...
        />
        {issueCoupon.data ? (
          <p className="mt-3 text-sm font-bold text-action-primary">
            회원 #{issueCoupon.data.member_id}에게 쿠폰 #{issueCoupon.data.coupon_id}을 발급했습니다.
          </p>
        ) : null}
        {issueCoupon.error ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(issueCoupon.error)}</p> : null}
      </ConsoleSection>
      <ConsoleSection className="mt-5" title="쿠폰 목록">
        <FilterPanel>
          ...
          <FilterField label="상태">
            <Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={status} onChange={(event) => setStatus(event.target.value)}>
          ...
          rows={filteredCoupons.map((coupon) => [
            ...
            <div key="status" className="flex flex-wrap items-center gap-1">
              {coupon.definition_status
                ...
                : <span className="rounded-full bg-surface-subtle px-2 py-1 text-xs font-bold text-content-secondary">정의 상태: 서버 미제공</span>}
  ...
  return (
    <ConsoleLayout title="관리자" subtitle="플랫폼 운영 콘솔" links={adminLinks}>
      <ConsoleHeader
        ...
        action={<Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">전체 상태</option><option value="ACTIVE">활성</option><option value="INACTIVE">비활성</option></Select>}
      />
      ...
      <ConsoleSection className="mt-5" title={sectionEditingID ? "홈 구좌 수정" : "홈 구좌 등록"}>
        <div className="grid gap-3 lg:grid-cols-[90px_1fr_1.2fr_1.1fr_130px_auto]">
          <Input type="number" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={sectionForm.sequence} onChange={(event) => setSectionForm((current) => ({ ...current, sequence: event.target.value }))} aria-label="구좌 순서" />
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={sectionForm.title} onChange={(event) => setSectionForm((current) => ({ ...current, title: event.target.value }))} placeholder="구좌명" aria-label="구좌명" />
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={sectionForm.description} onChange={(event) => setSectionForm((current) => ({ ...current, description: event.target.value }))} placeholder="설명" aria-label="구좌 설명" />
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={sectionForm.api_url} onChange={(event) => setSectionForm((current) => ({ ...current, api_url: event.target.value }))} placeholder="/api/v1/products/popular" aria-label="데이터 API 주소" />
          <Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={sectionForm.status} onChange={(event) => setSectionForm((current) => ({ ...current, status: event.target.value }))} aria-label="구좌 상태"><option value="ACTIVE">활성</option><option value="INACTIVE">비활성</option></Select>
        ...
        {saveHomeSection.error ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(saveHomeSection.error)}</p> : null}
        <div className="mt-4" />
        <DataTable
          columns={["순서", "구좌", "데이터 주소", "상태", "작업"]}
          rows={sortedSections.map((section) => [
            section.sequence,
            <div key="section"><p className="font-bold">{section.title}</p><p className="text-xs text-content-secondary">{section.description}</p></div>,
            section.api_url,
      ...
      <ConsoleSection className="mt-5" title={categoryEditingID ? "카테고리 수정" : "카테고리 등록"}>
        <div className="grid gap-3 lg:grid-cols-[1fr_1fr_1fr_100px_auto]">
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={categoryForm.name} onChange={(event) => setCategoryForm((current) => ({ ...current, name: event.target.value }))} placeholder="카테고리명" aria-label="카테고리명" />
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={categoryForm.slug} onChange={(event) => setCategoryForm((current) => ({ ...current, slug: event.target.value }))} placeholder="주소 식별자" aria-label="카테고리 주소 식별자" />
          <Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={categoryForm.parent_id} onChange={(event) => setCategoryForm((current) => ({ ...current, parent_id: event.target.value }))} aria-label="상위 카테고리">
          ...
          <Input type="number" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={categoryForm.display_order} onChange={(event) => setCategoryForm((current) => ({ ...current, display_order: event.target.value }))} aria-label="카테고리 순서" />
          <div className="flex gap-2">
        ...
        {saveCategory.error ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(saveCategory.error)}</p> : null}
        {deleteCategory.error ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(deleteCategory.error)}</p> : null}
        <div className="mt-4" />
        <DataTable
          columns={["순서", "카테고리", "주소 식별자", "단계", "작업"]}
      ...
      <ConsoleSection className="mt-5" title={editingID ? "이벤트 캐러셀 수정" : "이벤트 캐러셀 등록"}>
        <div className="grid gap-3 lg:grid-cols-[1.2fr_0.8fr_130px_110px]">
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="제목" aria-label="캐러셀 제목" />
          <Input className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.image_url} onChange={(event) => setForm((current) => ({ ...current, image_url: event.target.value }))} placeholder="이미지 URL" aria-label="이미지 URL" />
          <Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={form.target_type} onChange={(event) => setForm((current) => ({ ...current, target_type: event.target.value }))} aria-label="대상 유형"><option value="PRODUCT">상품</option><option value="MARKET">마켓</option><option value="URL">웹 주소</option></Select>
          <Input type="number" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.target_id} onChange={(event) => setForm((current) => ({ ...current, target_id: event.target.value }))} placeholder="대상 ID" aria-label="대상 ID" />
        </div>
        <div className="mt-3 grid gap-3 md:grid-cols-[120px_150px_1fr_1fr_auto]">
          <Input type="number" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.display_order} onChange={(event) => setForm((current) => ({ ...current, display_order: event.target.value }))} placeholder="순서" aria-label="노출 순서" />
          <Select className="h-11 rounded-control border border-border-interactive bg-surface-raised px-3 text-sm font-bold" value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))} aria-label="노출 상태"><option value="ACTIVE">활성</option><option value="INACTIVE">비활성</option></Select>
          <Input type="datetime-local" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.starts_at} onChange={(event) => setForm((current) => ({ ...current, starts_at: event.target.value }))} aria-label="시작 일시" />
          <Input type="datetime-local" className="h-11 rounded-control border border-border-interactive px-3 text-sm outline-none" value={form.ends_at} onChange={(event) => setForm((current) => ({ ...current, ends_at: event.target.value }))} aria-label="종료 일시" />
        ...
        {saveCarousel.error ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(saveCarousel.error)}</p> : null}
      </ConsoleSection>

      <div className="mt-5 grid gap-4 md:grid-cols-2">
        {filteredCarousels.map((carousel) => (
          <ConsoleSection key={carousel.id}>
            <div className="relative aspect-[16/7] overflow-hidden rounded-md bg-surface-subtle"><SafeImage src={carousel.image_url} alt="" fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" /></div>
            <div className="mt-3 flex items-start justify-between gap-3"><div><p className="font-bold">{carousel.title}</p><p className="mt-1 text-xs font-bold text-content-secondary">순서 {carousel.display_order ?? 0} · {carousel.link_url}</p><p className="mt-1 text-xs font-bold text-content-secondary">{cmsScheduleText(carousel.starts_at, carousel.ends_at)}</p></div><StatusBadge value={carousel.status} /></div>
