...
export function AdminHomeCardsPage() {
  ...
  return (
    <ConsoleLayout title="Admin" subtitle="플랫폼 운영 콘솔" links={adminLinks}>
      <ConsoleHeader
        title="홈 구좌 카드"
        description="텍스트 혜택 구좌와 통합 이미지·상품 구좌에 노출할 플랫폼 카드를 등록합니다. 활성 카드의 우선순위와 대상은 서버가 결정합니다."
      />
      <ConsoleSection className="mt-5" title="카드 등록" description="CTA 문구와 이동 URL은 함께 입력하세요. 쿠폰·이벤트 참조 ID는 해당 카드 유형에서만 사용됩니다.">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Field label="카드 유형">
            <select className={inputClass} value={form.cardType} onChange={(event) => setForm({ ...form, cardType: event.target.value as CardType, takeover: false })}>
              {cardTypeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            </select>
          </Field>
          <Field label="노출 대상">
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className="text-xs text-muted">등록 후 목록에서 활성화해야 홈에 노출됩니다.</span>
        </div>
      ...
      <ConsoleSection className="mt-5" title="등록 카드">
        <DataTable
          ...
          rows={cards.map((card) => [
            card.placement === "HOME_CONTEXT_TEXT" ? "텍스트 혜택" : "통합 카드",
...
function cardPayload(form: ReturnType<typeof emptyForm>) {
  ...
  return {
    placement: form.cardType === "EVENT" ? "HOME_FEATURE_CARD" : "HOME_CONTEXT_TEXT",
    ...
    is_takeover: form.cardType === "EVENT" && form.takeover,
