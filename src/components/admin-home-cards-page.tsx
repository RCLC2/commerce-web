...
const placementOptions: Array<{ value: Placement; label: string }> = [
  ...
  { value: "HOME_FEATURE_CARD", label: "홈·상품 상세 공통 카드" },
];
...
export function AdminHomeCardsPage() {
  ...
  return (
    <ConsoleLayout title="관리자" subtitle="플랫폼 운영 콘솔" links={adminLinks}>
      <ConsoleHeader
        title="홈 구좌 카드"
        description="홈 전용 텍스트 광고와 홈·상품 상세 리뷰 하단에 함께 사용하는 쿠폰·이벤트 카드를 등록합니다. 대상과 발급 이력을 반영한 실제 노출은 서버가 결정합니다."
      />
      <ConsoleSection className="mt-5" title="카드 등록" description="버튼 문구와 이동 URL은 함께 입력하세요. 쿠폰·이벤트 참조 ID는 해당 카드 유형에서만 사용됩니다.">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Field label="버튼 문구"><Input className={inputClass} value={form.ctaLabel} onChange={(event) => setForm({ ...form, ctaLabel: event.target.value })} placeholder="쿠폰 받기" /></Field>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <span className="text-xs text-content-secondary">등록 후 목록에서 활성화해야 해당 구좌에 노출됩니다.</span>
        </div>
        {create.isError ? <p className="mt-3 text-sm font-bold text-action-primary">{apiErrorMessage(create.error)}</p> : null}
      </ConsoleSection>
      ...
      <ConsoleSection className="mt-5" title="등록 카드">
        <DataTable
          ...
          rows={cards.map((card) => [
            ...
            card.headline,
            `${card.audience_type === "ALL" ? "전체" : "로그인 회원"} · ${card.priority}`,
        ...
        />
        {transition.isError ? <p className="mt-3 text-sm font-bold text-action-primary">{apiErrorMessage(transition.error)}</p> : null}
      </ConsoleSection>
    </ConsoleLayout>
  );
}
...
const inputClass = "h-11 w-full rounded-md border border-border-subtle bg-surface-raised px-3 text-sm font-bold outline-none focus:border-foreground";
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="grid gap-1 text-xs font-bold text-content-secondary">{label}{children}</label>;
}
