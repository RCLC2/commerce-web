...
import { ConsoleModal, consoleInputClass, useDebouncedValue } from "./console-ui";
...
export function SellerAdvertisingPage() {
  ...
  return (
    <ConsoleLayout title="판매자" subtitle="마켓 광고센터" links={sellerLinks}>
      <ConsoleHeader
        title="광고 캠페인"
      ...
      />
		{create.isError || transition.isError ? <p className="mt-3 text-sm font-bold text-status-negative">{apiErrorMessage(create.error ?? transition.error)}</p> : null}
      ...
      >
        ...
        <p className="mt-4 text-xs font-bold text-content-secondary">초안을 만든 후 검수 요청을 하면 운영자 승인 전까지 노출되지 않습니다.</p>
      </ConsoleModal>
...
export function AdminAdvertisingPage() {
  ...
  return (
    <ConsoleLayout title="관리자" subtitle="광고 운영 콘솔" links={adminLinks}>
          ...
          rows={campaigns.map((campaign) => {
            ...
            const controls = (
              <div key="actions" className="flex flex-wrap gap-2">
                ...
                {!["UNDER_REVIEW", "ACTIVE", "SCHEDULED", "PAUSED"].includes(status) ? <span className="text-xs font-bold text-content-secondary">검수 완료</span> : null}
              </div>
            );
            return [
              campaignName(campaign),
              `${displayLabel(campaignFormat(campaign))} · ${placementLabel(campaign.placement_key)}`,
...
function campaignRow(campaign: AdCampaign, transition: { mutate: (input: { id: number; action: "submit" | "pause" | "resume" }) => void; isPending: boolean }) {
  ...
  return [
    <div key="name"><p className="font-bold">{campaignName(campaign)}</p>{campaign.rejection_reason ? <p className="mt-1 text-xs font-bold text-action-primary">반려: {campaign.rejection_reason}</p> : null}</div>,
    `${displayLabel(campaignFormat(campaign))} · ${placementLabel(campaign.placement_key)}`,
...
function AdsAccessRequired({ role }: { role: string }) {
  return <ConsoleLayout title="광고센터" subtitle="광고센터" links={role === "관리자" ? adminLinks : sellerLinks}><ConsoleSection><h2 className="text-xl font-bold">{role} 권한이 필요합니다</h2><p className="mt-2 text-sm text-content-secondary">광고 캠페인과 가격표는 권한이 확인된 계정에서만 관리할 수 있습니다.</p></ConsoleSection></ConsoleLayout>;
}
...
const inputClass = consoleInputClass;
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="grid gap-1 text-xs font-bold text-content-secondary">{label}{children}</label>; }

function placementLabel(value: string) {
  return placementCatalog.find((placement) => placement.value === value)?.label ?? value;
}
