    ...
    <ConsoleLayout title="Admin" subtitle="광고 운영 콘솔" links={adminLinks}>
      ...
      <ConsoleSection className="mt-5" title="지면 가격표 등록" description="CPM은 1,000회 노출당 원화, 일 임대는 하루 원화 기준으로 입력합니다.">
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <Field label="광고 형식"><input readOnly value={rate.creativeFormat} className={inputClass} /></Field>
      ...
      <ConsoleSection className="mt-5" title="활성 가격표">
        <DataTable columns={["지면", "형식", "과금 방식", "단가", "상태"]} rows={rates.map((item) => [ratePlacement(item), item.creative_format, ratePricingModel(item), formatWon(ratePriceMicros(item)), item.status])} />
      </ConsoleSection>
