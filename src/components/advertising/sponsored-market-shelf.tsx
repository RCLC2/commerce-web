...
export function SponsoredMarketShelf({ decision, onNavigate }: { decision: AdDecision; onNavigate: () => void }) {
  ...
  return (
    <section className="relative rounded-2xl border border-action-primary/25 bg-surface-raised p-5 pt-12 shadow-sm">
      <SponsoredDisclosure />
      <div className="flex items-end justify-between gap-4">
        <div className="min-w-0">
          <Link href={decision.creative.landing_url} onClick={onNavigate} className="text-2xl font-bold hover:text-action-primary focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand">{market.name}</Link>
          {market.description ? <p className="mt-1 line-clamp-2 text-sm text-content-secondary">{market.description}</p> : null}
        </div>
      ...
      <div ref={carouselRef} className="no-scrollbar mt-5 flex snap-x gap-3 overflow-x-auto scroll-smooth pb-1">
        {market.products.map((product) => (
          <Link key={product.id} href={`/products/${product.id}`} onClick={onNavigate} className="group w-40 shrink-0 snap-start focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand md:w-48">
            ...
            <p className="mt-1 font-bold text-action-primary">{formatPrice(product.discount_price || product.base_price)}</p>
          </Link>
        ))}
      </div>
    </section>
