...
export function SponsoredMarketShelf({ decision, onNavigate }: { decision: AdDecision; onNavigate: () => void }) {
  ...
  return (
    <section className="relative rounded-2xl border border-brand/25 bg-white p-4 pt-10 shadow-sm">
      <SponsoredDisclosure />
      <div className="flex items-end justify-between gap-4">
        <div className="min-w-0">
          <Link href={decision.creative.landing_url} onClick={onNavigate} className="text-xl font-black hover:text-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand">{market.name}</Link>
