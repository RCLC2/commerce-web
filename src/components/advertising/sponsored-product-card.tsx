...
export function SponsoredProductCard({ decision, onNavigate }: { decision: AdDecision; onNavigate: () => void }) {
  ...
  return (
    <article className="relative overflow-hidden rounded-xl border border-action-primary/25 bg-surface-raised shadow-sm">
      <SponsoredDisclosure />
      <Link href={decision.creative.landing_url} onClick={onNavigate} className="group grid min-h-40 grid-cols-[128px_1fr] gap-4 p-3 pr-16 transition hover:bg-surface-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand md:grid-cols-[160px_1fr]">
        ...
        <div className="min-w-0 self-center py-2">
          ...
          <h2 className="mt-2 line-clamp-2 text-base font-bold leading-6 md:text-lg">{product.name}</h2>
          <div className="mt-3"><ProductCardPrice basePrice={product.base_price} discountPrice={product.discount_price} /></div>
          <p className="mt-2 text-xs font-bold text-action-primary">상품 상세로 이동</p>
        </div>
      </Link>
    </article>
  );
