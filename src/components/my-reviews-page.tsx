...
export function MyReviewsPage() {
  ...
  const reviews = useQuery({ queryKey: ["me-reviews", token], queryFn: () => api.listMyReviews(token), enabled: Boolean(token) });
  ...
  const update = useMutation({
    ...
    onSuccess: () => { setEditingID(null); void queryClient.invalidateQueries({ queryKey: ["me-reviews", token] }); },
  });
  const remove = useMutation({
    mutationFn: (id: number) => api.deleteReview(token, id),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["me-reviews", token] }),
  });

  if (!token) return <main className="mx-auto max-w-3xl px-4 py-16"><h1 className="text-2xl font-black">내 리뷰</h1><Link href="/login"><Button className="mt-5">로그인하기</Button></Link></main>;
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 pb-24 pt-8">
      ...
      {reviews.error ? <p className="mt-6 rounded-md bg-red-50 p-4 text-sm font-bold text-brand">{apiErrorMessage(reviews.error)}</p> : null}
      <div className="mt-6 space-y-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {editing ? (
                    <div className="mt-3 space-y-3">
                      ...
                      <div className="flex gap-2"><Button size="sm" onClick={() => update.mutate({ id: review.id })}>저장</Button><Button size="sm" variant="secondary" onClick={() => setEditingID(null)}>취소</Button></div>
                    </div>
                  ) : (
                    <>
                      ...
                      <div className="mt-3 flex gap-3 text-xs font-bold"><button onClick={() => { setEditingID(review.id); setDraft({ rating_x2: review.rating_x2 ?? Math.round(review.rating * 2), content: review.content }); }}>수정</button><button className="text-brand" disabled={remove.isPending} onClick={() => remove.mutate(review.id)}>삭제</button></div>
                    </>
                  )}
                </div>
              </div>
        ...
        {!reviews.isLoading && !items.length ? <p className="rounded-md border border-line bg-white p-8 text-center text-sm text-muted">작성한 리뷰가 없습니다.</p> : null}
      </div>
    </main>
  );
}
