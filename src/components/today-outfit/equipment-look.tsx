    ...
    <article className="w-full shrink-0 overflow-hidden rounded-3xl border border-action-primary/25 bg-surface-raised">
      <div data-testid="outfit-image-stage" className="relative min-h-[650px] overflow-hidden bg-[radial-gradient(circle_at_50%_42%,#fff_0_22%,#fff1f2_23%_51%,#f8fafc_72%)] px-2 pt-4 sm:min-h-[560px] sm:px-4">
        ...
        <div className="absolute left-1/2 top-[47%] h-[400px] w-[300px] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-[2rem] bg-surface-subtle shadow-[0_18px_32px_rgba(15,23,42,0.18)] sm:h-[440px] sm:w-[330px]">
          ...
          />
          <p id={disclosureID} className="absolute inset-x-2 bottom-2 rounded-full bg-content-primary/75 px-3 py-1.5 text-center text-xs font-bold text-content-inverse backdrop-blur">
        ...
        {look.items.map((item) => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block text-xs font-bold uppercase tracking-wide text-action-primary">{item.slot_label}</span>
              <strong className="mt-0.5 block truncate text-xs leading-4 sm:text-xs">{item.product.name}</strong>
              <span className="block text-xs text-content-secondary sm:text-xs">{formatPrice(item.product.discount_price || item.product.base_price)}</span>
            </span>
          </Button>
        ))}
      </div>
