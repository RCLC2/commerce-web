...
test("구매확정 주문 상품에 이미지를 첨부해 리뷰를 등록하고 상품 상세에서 확인한다", async ({ page }) => {
  ...
  await expect(page.getByText("리뷰 작성 완료")).toBeVisible();
  await expect(page.getByRole("link", { name: "내 리뷰 보기" })).toBeVisible();
...
test("서버가 지원하지 않는 이미지 MIME 타입은 presign 전에 거부한다", async ({ page }) => {
  await openReviewPanel(page);
  ...
  await expect(page.getByText("PNG, JPG, WEBP 이미지만 첨부할 수 있습니다")).toBeVisible();
  await expect(page.getByText("0/5")).toBeVisible();
});

test("10MB를 초과하는 리뷰 이미지는 presign 전에 거부한다", async ({ page }) => {
  await openReviewPanel(page);
  ...
  await expect(page.getByText("리뷰 이미지는 10MB 이하만 첨부할 수 있습니다")).toBeVisible();
  await expect(page.getByText("0/5")).toBeVisible();
});
...
test("내 리뷰 페이지에서 작성한 리뷰를 수정하고 삭제한다", async ({ page }) => {
  const reviewContent = `Playwright 관리 리뷰 ${Date.now()}`;
  ...
  let reviewArticle = page.locator("article").filter({ hasText: reviewContent });
  await expect(reviewArticle).toBeVisible();

  await reviewArticle.getByRole("button", { name: "수정" }).click();
  await reviewArticle.getByRole("button", { name: "수정 4점" }).click();
  await reviewArticle.getByLabel("리뷰 수정 내용").fill(updatedContent);
  await reviewArticle.getByRole("button", { name: "저장" }).click();

  reviewArticle = page.locator("article").filter({ hasText: updatedContent });
  await expect(reviewArticle).toBeVisible();
